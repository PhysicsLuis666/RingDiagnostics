...
parser.add_argument("--current", type=int, default=1400)
...
output_dir1 = os.path.join("outputs3-1", timestamp)
os.makedirs(output_dir1, exist_ok=True)

output_dir2 = os.path.join("outputs3-2", timestamp)
...
#set up dt to call for waveform
#-----------------------------------------------------------------------------
def dtime(sync_part, ti, tf):
   
    sqrt = np.sqrt
    time = sync_part.time()
    
    dt = sqrt((time-ti)/(tf-ti))
...
minipulses_per_pulse = 980       #1044, 981
...
#set new angular deflection limits
#--------------------------------------------------------------------------------------
HKick14 = 12.26E-03   #based on theta_2 = theta_1 * p1/p2 * I2/I1 @ 1.3 GeV
VKick14 = 12.25E-03   
HKick23 = 6.80E-03   
VKick23 = 6.79E-03   

#all work in output folder output-0 was using old Hkick and Vkick values at 1.0 GeV I mistakenly
#thought i put the right scaled deflections
...
strength_hkicker10 = 14.04e-03  #1 h
strength_vkicker10 =  8.84e-03  #1 v
strength_hkicker11 = -4.28e-03  #2 h
strength_vkicker11 = -5.06e-03  #2 v
strength_hkicker12 = -4.36727974875e-03 #3h
strength_vkicker12 = -5.32217284098e-03 #3v
strength_hkicker13 = 14.092989681e-03 #4h
strength_vkicker13 =  9.0098984536e-03 #4v

#obtain limiting constants
# --------------------------------------------------------------------------------------
cH10 = strength_hkicker10/HKick14  #H1  horizontal kicker # 1
cV10 = strength_vkicker10/VKick14  #V1  vertical kicker   # 1
cH12 = strength_hkicker11/HKick23  #H2  horizontal kicker # 2
cV12 = strength_vkicker11/VKick23  #V2  vertical kicker   # 2
cH13 = strength_hkicker12/HKick23  #H3  horizontal kicker # 3
cV13 = strength_vkicker12/VKick23  #V3  vertical   kicker # 3
cH14 = strength_hkicker13/HKick14  #H4  horizontal kicker # 4
cV14 = strength_vkicker13/VKick14  #V4  vertical kicker   # 4

# now i have to make sure that all of these are bellow 1 or equal to 1
#--------------------------------------------------------------------------------------
bump_array = np.array([cH10,cV10, cH12, cV12, cH13, cV13, cH14, cV14])

if np.any(bump_array > 1):
    max_bump = np.max(bump_array)
    scalefactor = 1/max_bump
    bump_array = bump_array/max_bump

#------------------------------------------------------
#obtain new max strengths
#------------------------------------------------------
strength_hkicker10 = HKick14 * bump_array[0]  #H1
strength_hkicker13 = HKick14 * bump_array[6]  #H4
strength_vkicker10 = VKick14 * bump_array[1]  #V1
strength_vkicker13 = VKick14 * bump_array[7]  #V4
strength_hkicker11 = HKick23 * bump_array[2]  #H2
strength_hkicker12 = HKick23 * bump_array[4]  #H3
strength_vkicker11 = VKick23 * bump_array[3]  #V2  
strength_vkicker12 = VKick23 * bump_array[5]  #V3
...
tih = 0         #may want to set to zero to see if its same as control room -0.0
tiv = 0         #-0.000484#0.0      #-.002, 
tf =  0.001
#si =  1.0  # we split the si into two values for H and V kickers
sih = 0.99
siv = 1.3   #.77 #99
sfh = 0.50  #.36 #.43 #.45, #.43, #43, #.47 works nice#30 new point after setting tiv = tih = 0.0
sfv = 0.30 #.418 #37, #.38, .40, .43
...
start_time = time.time()   #ive only added the output of creating a new file with the details of the beam measures etc
...
class Monitor:
    ...
    def __call__(self, params_dict:dict) -> None:
        ...
        node = params_dict["node"] #current node im tracking through


        # Get bunch
        bunch = params_dict["bunch"]
        
        twiss_calc1 = BunchTwissAnalysis()
        twiss_calc1.computeBunchMoments(bunch, 2, 0, 0) 
        
        xcentroid = twiss_calc1.getAverage(0)*1000
        xpcentroid= twiss_calc1.getAverage(1)*1000
        ycentroid = twiss_calc1.getAverage(2)*1000
        ypcentroid= twiss_calc1.getAverage(3)*1000
        
        self.coordinatex.append(xcentroid)
        self.coordinatey.append(ycentroid)
        self.positions.append(position)
        self.coordinatexp.append(xpcentroid)
        self.coordinateyp.append(ypcentroid)

        """       
        TLength = lattice.getLength()
        HLength = TLength / 2
        ...
        for node, (start_pos, _) in node_positions.items():
            diff = abs(start_pos - HLength)
            if diff < min_diff:
                min_diff = diff
                halfway_node = node
        
        print(f"Closest node to halfway is: {halfway_node.getName()}")
        """
...
#monitor1 = MonitorTEAPOT("monitor1")
#lattice.addNode(monitor, 0)
#lattice.initalize()
#nodes1 = lattice.getNodes()

lattice.trackBunch(bunch, actionContainer=action_container) 
# Convert to NumPy array if it's a list
#x = np.array(monitor.coordinatex)
# Define threshold (e.g., 0.01 mm)
#threshold = 0.01  # mm
# Set all values with abs(x) < threshold to zero
#x[np.abs(x) < threshold] = 0.0
# Optional: assign back to your object
#monitor.coordinatex = x.tolist()

plt.figure(figsize=(10, 6))
plt.plot(monitor.positions, monitor.coordinatex, label = f"centroid x sih = {sih}, siv = {siv} , sfh = {sfh}, sfv = {sfv}", color = "black")
plt.plot(monitor.positions, monitor.coordinatey, label = f"centroid y sih = {sih}, siv = {siv}, sfh = {sfh}, sfv = {sfv}", linestyle = "--", color = "green")
#plt.xticks(np.arange(0, monitor.positions[-1],15))
#plt.yticks(np.arange(np.min(monitor.coordinatex), np.max(monitor.coordinatex), 10))
plt.xlim(monitor.positions[0],monitor.positions[-1])
plt.xlabel("position [m]")
plt.hlines(0, 0, 248, label = "y=0", linestyle="--")
plt.ylabel("centroid x and y [mm]")
plt.grid()
plt.legend()
plt.show()
"""
...
# Convert your list to NumPy arrays
x_arr = np.array(monitor.coordinatex)
y_arr = np.array(monitor.coordinatey)
s_arr = np.array(monitor.positions)
...
x_swapped = np.concatenate((x_arr[half:], x_arr[:half]))
y_swapped = np.concatenate((y_arr[halfy:], y_arr[:halfy]))
s_swapped = np.concatenate((s_arr[half:], s_arr[:half]))
s_new = s_arr -np.max(s_arr)/2

# Normalize position so it starts at 0
s_swapped = s_swapped - s_swapped[0]

# Plot the result
plt.figure(figsize=(10, 6))
plt.plot(s_new, x_swapped, label="Centroid X (halves swapped)", color = "black")
plt.plot(s_new, y_swapped, label="Centroid Y (halves swapped)", linestyle = "--", color = "green")
#plt.xticks(np.arange(np.min(s_new), np.max(s_new), 10))
#plt.yticks(np.arange(np.min(x_swapped), np.max(x_swapped), 10))
plt.hlines(0, -124, 124, label = "y = 0")
#plt.xlim(-20,20)
plt.xlabel("Path Length [m]")
plt.ylabel("Centroid X [mm]")
plt.title("Centroid X with Halves Swapped")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
for turn_index in range(args.turns):
    ...
    #obtain charge density grid
    if args.sc_xy:
        charge_density = sc_calc.getRhoGrid()
        potential_grid = sc_calc.getPhiGrid()
        longitudinal_density = sc_calc.getLongGrid()
    ...
    e_x = twiss_calc.getEmittance(0)*1000000
    e_y = twiss_calc.getEmittance(1)*1000000
    e_z = twiss_calc.getEmittance(2)
    
    centroidx = twiss_calc.getAverage(0)
    centroidxp = twiss_calc.getAverage(1)
    centroidy = twiss_calc.getAverage(2)
    centroidyp= twiss_calc.getAverage(3)
    
    centroid_x.append(centroidx)
    centroid_y.append(centroidy)
    centroid_xp.append(centroidxp)
    centroid_yp.append(centroidyp)
    
    hs1 = hkickerwave.getStrength()
    vs1 = vkickerwave.getStrength()

    hs.append(hs1)
    vs.append(vs1)
    
    time_array.append(sync_part.time())
    ...
    if mpi_rank == 0:
        ...
        message += "centroid x={:0.2f} mm ".format(centroidx*1000)
        message += "centroid y={:0.2f} mm".format(centroidy*1000)
        message += "centroid xp={:0.2f}mrad ".format(centroidxp*1000)
        message += "centroid yp={:0.2f} mrad".format(centroidyp*1000)
        print(message)
        sys.stdout.flush()
    ...
    if (turn_index % args.write_bunch_freq == 0) or (turn_index == args.turns - 1):
        ...
        filename = os.path.join(output_dir1, filename)
        if mpi_rank == 0:
            print(filename)
            sys.stdout.flush()
    ...
    if args.sc_xy:   
        if (turn_index % args.write_bunch_freq == 0) or (turn_index == args.turns - 1):
            filename_n = "n_density_{:04.0f}".format(turn_index)
            filename_n = os.path.join(output_dir2,filename_n)
            
            sizeX = charge_density.getSizeX()
            sizeY = charge_density.getSizeY()
            rho = np.zeros((sizeX, sizeY))
            for i in range(sizeX):
                for j in range(sizeY):
                    rho[i,j] = charge_density.getValueOnGrid(i,j)
            np.savetxt(filename_n, rho)
...
turns1 = np.arange(0, args.turns)
plt.plot(turns1, centroid_x, label="Centroid X")
plt.plot(turns1, centroid_y, label="Centroid Y", linestlye="--", color="orange")

# Add foil band
#plt.axhspan(foil_xmin, foil_xmax, color='red', alpha=0.2, label="Foil X Range")

plt.xlabel("Turn")
plt.ylabel("Centroid X & Y [m]")
plt.title("Centroid Coordinates vs Turn")
plt.legend()
plt.grid(True)
plt.show()

# Your data array (e.g., NewArr = np.array([...]))
filename = "nullwaeveform"  # No extension
filepath = os.path.join(output_dir3, filename)
NewArr = np.column_stack((hs, vs, time_array))

# Save the file
np.savetxt(filepath, NewArr, fmt='%.6f')
...
log_data ={
    ...
    "tiv": tiv,
    "tf":  tf,
    "minipulse_intensity": minipulse_intensity,
    "cpu time": total_cpu_time,
    "wall clock time":time_ellapsed,
    "sih":sih,
    "siv":siv,
    'sfh':sfh,
    'sfv':sfv,
    "Twiss emittance x": e_x,
    "Twiss emittance y": e_y,
    "Twiss emittance z": e_z,
    "intensity":intensity,
    "rms x": xrms[-1],
    "rms y": yrms[-1],
...
subfolder_name = "Analysis1-1"
subfolder_path = os.path.join(output_dir1, subfolder_name)
os.makedirs(subfolder_path, exist_ok = True)

args_output_path = os.path.join(subfolder_path, 'args_output.txt')
